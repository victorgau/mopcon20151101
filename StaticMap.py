...
class StaticMap:
    def __init__(self, width=640, height=480, center=(25.042185, 121.614548), zoom=14, mapType="OSM"):
        self.width = width
        self.height = height
        self.zoom = zoom
        self.center = center
        ...
        self.NWDeg = self.xy2deg((self.Cxy[0]-width/2,self.Cxy[1]-height/2),zoom,False)
        self.SEDeg = self.xy2deg((self.Cxy[0]+width/2,self.Cxy[1]+height/2),zoom,False)
        self.mapImage =  Image.new("RGB", (width, height), "black")
        self.centerImage = Image.open("purpleflag.png")
        self.markerImage = Image.open("pushpin.png")
    ...
    def createBaseMap(self):
        ...
        while i < tl + tr:
            ...
            y = ty - tu
            while j < td + tu:
                imgurl = self.getMapTileUrl(x+i, y+j)
    ...
    def getMapTileUrl(self, x, y):
        
        imgurl = {
            "StamenTerrain" : lambda: "http://tile.stamen.com/terrain/{z}/{x}/{y}.jpg".format(z=self.zoom,x=x, y=y),
            "OSM"  : lambda: "http://{a}.tile.openstreetmap.org/{z}/{x}/{y}.png".format(a='a',z=self.zoom,x=x, y=y),
            "GoogleSatellite"  : lambda: "http://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}".format(z=self.zoom,x=x, y=y),
            "Satellite"  : lambda: "http://otile1.mqcdn.com/tiles/1.0.0/sat/{z}/{x}/{y}.jpg".format(z=self.zoom,x=x, y=y),
        }.get(self.mapType, lambda: "http://{a}.tile.openstreetmap.org/{z}/{x}/{y}.png".format(a='a',z=self.zoom,x=x, y=y))()

        return imgurl

    def setMapType(self, mapType="OSM"):
        self.mapType = mapType
